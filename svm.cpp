...
#pragma omp parallel for private(j) schedule(guided)			   // Added to take advantage of multi-core
		for (j = start; j < len; j++)
...
Solver s;
s.Solve(l, ONE_CLASS_Q(*prob, *param), zeros, ones, alpha, 1.0, 1.0, param->eps,
		si, param->shrinking);
...
#pragma omp parallel for private(i) reduction(+:sum) schedule(guided)		// Take advantage of multicore
	for (i = 0; i < model->l; i++)
...
#pragma omp parallel for private(i) schedule(guided)		  // take advantage of multicore		
	for (i = 0; i < l; i++)
...
static const char *kernel_type_table[] = { "linear", "polynomial", "rbf",
	"sigmoid", "precomputed", "external", NULL };
